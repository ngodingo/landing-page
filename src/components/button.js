...
export function Button ({text, href, theme, className}) {
  ...
  const defaultClassName = {
    common: 'block w-content h-12 px-6 py-2.5 rounded-3xl font-lexend-deca font-body-2 font-bold box-border duration-[50ms]',
    normal: 'mt-1 ml-1',
  ...
  return (
    <div className="group">
      <Link 
        href={href} 
        className={`${getDefaultClassName()} ${getBtnTheme()} ${className}`} >
          {text}
      </Link>
    </div>
  )
